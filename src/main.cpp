...
#if defined(NDEBUG)
# error "Ncccoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7268d7a1d85ad57c360ba39f3761c1d6226a7fdb3e23203d4e8ca2be12df6172");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ncccoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ncccoin Signed Message:\n";
...
{
    // Ncccoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2628000); // Ncccoin: 10 years, BlockHalving

    if (nHeight <= 1)
        return 5500000000 * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ncccoin: 1 day, BlockDifficulty
static const int64 nTargetSpacing = 2 * 60; // Ncccoin: 2 minutes, BlockTime
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ncccoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ncccoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[1] = 0xc2; //c1
        pchMessageStart[2] = 0xba; //b7
        pchMessageStart[3] = 0xd7; //dc
        hashGenesisBlock = uint256("0x4189281fc3180f9d16acb38e69a756933c7037cba61e4f650a12cbb0a5f8adbd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NCCCOIN WAS BORN ON 15/AUG/2018.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04128cbb4146a7680bb77f874f46dfe9e5b8ceb4d771533506e23019a96e2cdd43bbb0b24ce25b7cc064d34ce521188472835300ff441383b049985adeaf489ead") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534851140;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087280339;

        if (fTestNet)
        {
            block.nTime    = 1534851112;
            block.nNonce   = 395073980;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xef8aba81a441b6bf80e1affa0f21b99861168c10e3df65b69f31f730e7de41c7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc6, 0xbb, 0xd1 }; // Ncccoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NcccoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NcccoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NcccoinMiner(CWallet *pwallet)
{
    printf("NcccoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ncccoin-miner");
    ...
    try { loop {
        ...
        printf("Running NcccoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NcccoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NcccoinMiner, pwallet));
}
